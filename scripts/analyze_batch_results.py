...
def compute_detailed_metrics(results: List[Dict], config: Dict = None) -> Dict[str, Any]:
    ...
    for i in range(num_agents):
        agent_id = first_debate['rounds'][0][i]['agent_id'] if first_debate['rounds'] else f"agent_{i}"
        ...
        for round_idx, round_responses in enumerate(debate['rounds']):
            for response in round_responses:
                ...
                if round_idx > 0 and len(debate['rounds']) > 1:
                    prev_answer = debate['rounds'][round_idx - 1][round_responses.index(response)]['answer']
    ...
    for agent_id, stats in agent_stats.items():
        if stats['total_responses'] > 0:
            stats['avg_response_length'] = stats['total_reasoning_length'] / stats['total_responses']
...
def visualize_metrics(metrics: Dict[str, Any], output_dir: Path):
    ...
    axes[1, 0].text(0.5, 0.95, f"Avg Improvement: {avg_imp:+.1%}",
    ...
    axes[1, 1].text(quality + 0.05, 0, f"{quality:.1%} - {quality_text}",
                    va='center', fontweight='bold', color=quality_color)
...
def analyze_results():
    ...
    print(f"Completed: {metrics['completed_debates']} ({metrics['completed_debates']/metrics['total_debates']:.1%})")
    ...
    print(f"Kept for training: {metrics['kept_for_training']} ({metrics['kept_for_training']/total:.1%})")
    print(f"Filtered (unanimous): {metrics['filtered_out_unanimous']} ({metrics['filtered_out_unanimous']/total:.1%})")
    print(f"Filtered (ambiguous <0.5): {metrics['filtered_out_ambiguous']} ({metrics['filtered_out_ambiguous']/total:.1%})")
    ...
    if metrics['kept_for_training'] > 0:
        print(f"Pairs per kept debate: {metrics['dpo_pairs_generated']/metrics['kept_for_training']:.1f}")
    ...
    print(f"\nCATEGORY BREAKDOWN")
    print(f"{'─'*80}")
    category_stats = defaultdict(lambda: {'total': 0, 'kept': 0, 'unanimous': 0, 'ambiguous': 0, 'pairs': 0})
    ...
    targets = [
        ...
        ("Convergence (improved) >50%", convergence['improved']/total if total > 0 else 0, 0.5, "higher"),
